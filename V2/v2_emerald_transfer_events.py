...
EMERALD_V2_ABI=config.get("EMERALD_V2_ABI")
EMERALD_V2_ABI=config.get("EMERALD_V1_ABI")
...
if __name__ == '__main__':
    ...
    for p in range(0,num_process):
        ...
        if(num_process == 1):
            parse_blocks(EMERALD_V2_ADR, EMERALD_V2_ABI, first_block, last_block, p, queue)
        else:
            print(f"Worker #{p} will process from {first_block} to {last_block}")
            process = Process(target=parse_blocks, args=(EMERALD_V2_ADR, EMERALD_V2_ABI, first_block, last_block, p, queue))   
    ...
    with open("./v2_transfer_events.json", "w") as file:
        json.dump(list(transfer_events), file)

    #build ledger
    transfer_ledger = {}
    #key: address
    #value: (in_tx_count, out_tx_count, balance,[tx_hash],[inbound_txs],[outbound_txs])
    for event in transfer_events:
        ...
        sender_inbound_txs = transfer_ledger[sender][4]
        sender_outbound_txs = transfer_ledger[sender][5] + [tx_hash]
        transfer_ledger[sender] = (sender_in_tx, sender_out_tx, sender_balance,sender_txs,sender_inbound_txs,sender_outbound_txs)
        ...
        receiver_inbound_txs = transfer_ledger[receiver][4] + [tx_hash]
        receiver_outbound_txs = transfer_ledger[receiver][5]
        transfer_ledger[receiver] = (receiver_in_tx, receiver_out_tx, receiver_balance, receiver_txs,receiver_inbound_txs,receiver_outbound_txs)

    #store TRANSFER ledger events 
    with open("./v2_transfer_ledger.json", "w") as file:
