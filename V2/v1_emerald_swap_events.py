...
def parse_blocks(contract_address, contract_abi, initial_block, last_block, worker_id, queue):  
    swap_events = set()
    contract_v3 = w3.eth.contract(address=w3.to_checksum_address(contract_address), abi=contract_abi)  
    contract_v2 = w3.eth.contract(address=w3.to_checksum_address("0xa51d3b97764c3b96f364218d8a6fd047ebe9b349"), abi=contract_abi)  
    for x in range(initial_block, last_block+1): 
        events = contract_v3.events.Swap.get_logs(fromBlock=x, toBlock=x)
        if (events!=()):
            for event in events:  
        ...
        events = contract_v2.events.Swap.get_logs(fromBlock=x, toBlock=x)
        if (events != ()):
            for event in events:   
                tx_hash = event["transactionHash"].hex()  
                block = event["blockNumber"]
