...
def parse_blocks(contract_addresses, contract_abi_v3, contract_abi_v2, initial_block, last_block, worker_id, queue):  
    swap_events = set()
    contract_pool1 = w3.eth.contract(address=w3.to_checksum_address(contract_addresses[0]), abi=contract_abi_v3)  
    contract_pool2 = w3.eth.contract(address=w3.to_checksum_address(contract_addresses[1]), abi=contract_abi_v2)
    for x in range(initial_block, last_block+1): 
        events = contract_pool1.events.Swap.get_logs(fromBlock=x, toBlock=x)
        if (events!=()):
            for event in events:  
        ...
        if (events != ()):
            for event in events:   
                ...
                emerald_amount = event["args"]["amount1Out"]-event["args"]["amount1In"]
                eth_amount = event["args"]["amount0Out"]-event["args"]["amount0In"] 
                swap_events.add((tx_hash,block,sender,receiver,emerald_amount,eth_amount)) 
        if(x % 300 == 0):
...
EMERALD_V2_FBLOCK=int(config.get("EMERALD_V2_FBLOCK"))
EMERALD_V2_LBLOCK=int(config.get("EMERALD_V2_LBLOCK"))

UNI_POOL_ADRRESES = config.get("V2_UNI_POOL_ADDR").split(",")
UNI_V3_POOL_ABI = config.get("V3_UNI_POOL_ABI")
UNI_V2_POOL_ABI = config.get("V2_UNI_POOL_ABI")
...
if __name__ == '__main__':
    ...
    for p in range(0,num_process):
        first_block = EMERALD_V2_FBLOCK + (block_step * p)
        last_block = first_block + block_step  - 1
        if(num_process == 1):
            parse_blocks(UNI_POOL_ADRRESES, UNI_V3_POOL_ABI, UNI_V2_POOL_ABI, first_block, last_block, p, queue)
        else:
            print(f"Worker #{p} will process from {first_block} to {last_block}")
            process = Process(target=parse_blocks, args=(UNI_POOL_ADRRESES,  UNI_V3_POOL_ABI, UNI_V2_POOL_ABI, first_block, last_block, p, queue))   
    ...
    with open("./v2_swap_events.json", "w") as file:
    ...
    with open("./v2_swap_ledger.json", "w") as file:
